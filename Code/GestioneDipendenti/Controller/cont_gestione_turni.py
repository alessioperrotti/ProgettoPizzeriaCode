...
class ContGestioneTurni(object):
    ...
    def update_tabella(self):#se inserisco il cuoco non va,se inserisco il cameriere si, se inserisco entrami ok
                                #forse funziona ora...boo
        # self.view.tabella.clearContents()
        for i in range(self.view.tabella.rowCount()):
            for j in range(self.view.tabella.columnCount()):
                ...
                self.view.tabella.setItem(i, j, item_vuoto)
        # Crea una mappatura per i tipi di turno agli indici di riga
        mappatura_turni = {"Pranzo": 0, "Cena": 1, "Pranzo & Cena": [0, 1]}

        for persona in self.model.lista_cuochi + self.model.lista_camerieri:
            for i, turno in enumerate(persona.turno):
                item_vuoto = QTableWidgetItem("vuoto")
                item_vuoto.setTextAlignment(Qt.AlignmentFlag.AlignCenter)
                item_nome = QTableWidgetItem(persona.nome + " " + persona.cognome)
